...
> 🧭 **AI 운영/회사 적용은 `docs/AI-GUIDE.md`를 먼저 따른다** — 환경 인벤토리, 3가지 오케스트레이션 방식,
> **모델 등급별 전략(SOTA=방식 A / 약한 오픈모델=방식 B+검증)**, 플레이북(프로필·위키·cron·칸반 파이프라인·업데이트),
> 그리고 **문서 유지 rules**(§7). 에이전트/크론/파이프라인/모델/결정이 바뀌면 같은 작업 단위에서
> `docs/AI-GUIDE.md`(인벤토리)와 관련 문서를 함께 갱신한다. 리서치 근거는 `docs/RESEARCH-multi-agent.md`.
