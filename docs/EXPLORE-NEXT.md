...
## 7. [리서치] 멀티에이전트 방식 — 남들은 뭘 쓰나 + 모델 등급별 전략
- 상세: **`docs/RESEARCH-multi-agent.md`**
- 요약:
  - 프레임워크: LangGraph(프로덕션 1위)·CrewAI(역할 crew, 프로토타입)·AutoGen(연구). 코딩툴은 subagent 내장 흐름.
  - 패턴: Orchestrator-Worker(최다) / Sequential Pipeline(=architect→…→reporter) / Hierarchical(Paperclip류).
  - **SOTA 모델** → 강한 단일 에이전트 + 위임(방식 A)이 정석(5역할 조립라인은 주류 아님).
  - **약한 오픈모델(회사용: Gemma/Qwen/MiMo/Kimi)** → **신뢰도 복리 붕괴**(0.9⁵≈59%) 때문에 정반대:
    **결정적 워크플로우 + 좁은 역할 분해 + 스텝별 검증 게이트 + 짧은 체인 + 강 tool-calling 모델**(방식 B/Paperclip). 약한 모델에 긴 자율 위임 = 지양.
- **회사 적용 PoC 방향**: 약한 오픈모델 1개로 "좁은 역할 + 검증 게이트" 파이프라인(방식 B).

---
> 진행할 때: 하나씩 골라서 요청하면 그 항목만 통제된 방식으로 세팅.
> 지금 상태: #1·#2·#3 완료(위키 git+주간 lint 포함). 다음 후보 = #6 Paperclip PoC / #4 네이티브 칸반 파이프라인.
> **모델 등급 주의**: 집(SOTA)=방식 A / 회사(약한 오픈모델)=방식 B+검증(리서치 §7 참조).
