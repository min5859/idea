...
## Phase 7 — 진짜 멀티 agent 단톡방 (백로그)
> 현재 Group Room = 방식 A(단일 리더가 일회용 서브에이전트에 위임·취합). @mention은 리더에게 주는 힌트일 뿐, 멘션한 다른 agent가 직접 응답하진 않음.
> 목표: 한 방에서 `@A`→A가, `@B`→B가 각자(=각자 다른 게이트웨이/프로필) 응답하는 진짜 그룹 채팅.

- [ ] **접근 결정**: ①메시지별 게이트웨이 라우팅 vs ②네이티브 칸반(방식 B) 기반 — *추천 ②*(이름 붙은 영속 봇 협업, "여러 도메인 특화 봇" 비전에 부합. Phase 5에서 칸반 read는 깔아둠)
- [ ] (①안) 채팅 전송 경로에 per-메시지 baseUrl 주입 → 멘션된 agent의 게이트웨이로 라우팅. ⚠️침습 큼(단일 `HERMES_API` 묶임, ~24파일), 업스트림 머지 부담 — Phase 2에서 비목표로 미뤘던 "동시 멀티룸"
- [ ] (②안) 멘션 → 네이티브 칸반 태스크 생성(assignee=멘션 agent) → 담당 봇이 claim·응답 → 방에 결과 표시. 칸반 write 경로 필요(Phase 5 v1은 read만)
- [ ] 방 멤버(참여 agent) 관리 UI + 각 agent 응답을 발신자 구분된 말풍선으로
- [ ] 멀티 agent 응답이 한 방에 시간순으로 섞여 보이는지 실측
